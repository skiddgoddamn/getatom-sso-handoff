...
Как это устроено: getatom выпускает короткоживущий подписанный токен (JWT) и передаёт его вашему сервису через redirect. В токене — стабильный ID пользователя, имя и фамилия, аватар, подтверждённые email и телефон: всё, что нужно, чтобы создать аккаунт и открыть сессию без повторной верификации контактов. Вам не нужно делать свою систему авторизации, хранить пароли или слать коды подтверждения.
...
Демо из [examples/vendor-node/](examples/vendor-node/) — до и после клика по кнопке:

| Лендинг сервиса | После SSO-цикла |
|---|---|
| ![Кнопка «Войти через getatom»](docs/img/landing.png) | ![Пользователь авторизован](docs/img/authorized.png) |

Между этими двумя экранами — один клик: выпуск токена на getatom, проверка подписи у вас и создание вашей сессии происходят за доли секунды внутри цепочки редиректов.
...
| **Контакты уже подтверждены** | Email и телефон в токене проверены getatom — аккаунт создаётся верифицированным, пользователю не нужно подтверждать их повторно |
